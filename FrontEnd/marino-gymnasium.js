fetch('http://54.152.20.175:5000/')
    ...
    .then((json) => {
        let sun = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Sunday'
        );
        let mon = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Monday'
        );
        let tue = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Tuesday'
        );
        let wed = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Wednesday'
        );
        let thu = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Thursday'
        );
        let fri = json.filter((item) => item.Location === 'Marino Gymnasium'
                                        && item.Weekday === 'Friday'
        );
        let sat = json.filter((item) => item.Location === 'Marino Gymnasium'
        ...
        var layout = {
            title: 'Marino Center Gymnasium Average Capacity Count',
        };
        ...
        Plotly.newPlot('marino-gymnasium', data, layout, config);
    });
